...
dwv.image.Image = function(geometry, buffer, numberOfFrames) {
  ...
  // initialise RSIs
  for (
    var s = 0, nslices = geometry.getSize().getNumberOfSlices();
    s < nslices;
    ++s
  ) {
   ...
   * @type Number
   */
  var numberOfComponents = buffer[0].length / geometry.getSize().getTotalSize();
  /**
   * Meta information.
   * @private
   * @type Object
   ...
   */
  this.setFirstOverlay = function(over) {
    overlays[0] = over;
  };
        ...
        for (var j = 0; j < nrows; j++) {
          for (var i = 0; i < ncols; i++) {
            ...
            for (var wi = 0; wi < 9; ++wi) {
              newValue +=
                this.getValueAtOffset(pixelOffset + wOffFinal[wi], f) *
                weights[wi];
            }
...
dwv.image.Image.prototype.quantifyRect = function(rect) {
  ...
  // surface
  var spacing = this.getGeometry().getSpacing();
  var surface = rect.getWorldSurface(
    spacing.getColumnSpacing(),
    spacing.getRowSpacing()
  );
  if (surface !== null) {
...
dwv.image.Image.prototype.quantifyEllipse = function(ellipse) {
  ...
  // surface
  var spacing = this.getGeometry().getSpacing();
  var surface = ellipse.getWorldSurface(
    spacing.getColumnSpacing(),
    spacing.getRowSpacing()
  );
  if (surface !== null) {
