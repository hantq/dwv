    ...
    for (var j = 0; j < canvas.height; ++j) {
      ...
      for (var i = 0; i < canvas.width; ++i) {
        if (c <= xMin) {
          ...
          y =
            ((c - (windowCenter - 0.5)) / (windowWidth - 1) + 0.5) *
              (yMax - yMin) +
            yMin;
          y = parseInt(y, 10);
        }
      ...
      for (n = 0; overlays[n]; n++) {
        if (overlays[n].value === 'window-center') {
          if (event.type === 'wl-center-change') {
            ...
            var wcStr = dwv.utils.replaceFlags2(overlays[n].format, [
              Math.round(event.wc),
            ]);
            if (li) {
          ...
          if (event.type === 'wl-width-change') {
            ...
            dwv.html.cleanNode(li);
            var wwStr = dwv.utils.replaceFlags2(overlays[n].format, [
              Math.round(event.ww),
            ]);
            if (li) {
          ...
          if (event.type === 'position-change') {
            ...
            dwv.html.cleanNode(li);
            var valueStr = dwv.utils.replaceFlags2(overlays[n].format, [
              event.value,
            ]);
            if (li) {
          ...
          if (event.type === 'position-change') {
            ...
            dwv.html.cleanNode(li);
            var posStr = dwv.utils.replaceFlags2(overlays[n].format, [
              event.i,
              event.j,
              event.k,
            ]);
          ...
          if (event.type === 'frame-change') {
            ...
            dwv.html.cleanNode(li);
            var frameStr = dwv.utils.replaceFlags2(overlays[n].format, [
              event.frame,
            ]);
            if (li) {
