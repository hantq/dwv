...
dwv.State = function() {
  ...
  this.fromJSON = function(json) {
    ...
    if (data.version === '0.1') {
      ...
      throw new Error(
        "Unknown state file format version: '" + data.version + "'."
      );
    }
    return res;
  };
  ...
  this.apply = function(app, data) {
    ...
    app
      .getViewController()
      .setWindowLevel(data['window-center'], data['window-width']);
...
dwv.v02Tov03Drawings = function(drawings) {
  ...
  var groupDrawings =
    typeof drawings === 'string' ? JSON.parse(drawings) : drawings;
    ...
    for (var f = 0, lenf = inputDrawings[k].length; f < lenf; ++f) {
      ...
      for (var g = 0, leng = drawGroups.length; g < leng; ++g) {
        ...
        if (drawGroup.name() === 'ruler-group') {
          ...
          drawGroup.name() === 'protractor-group' ||
          drawGroup.name() === 'rectangle-group'
        ) {
          quant = {
            angle: {
