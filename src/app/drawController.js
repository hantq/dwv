...
dwv.DrawController = function(drawDiv) {
  ...
  this.zoomStage = function(scale, scaleCenter) {
    ...
    var newOffsetX =
      scaleCenter.x / oldScale.x + oldOffset.x - scaleCenter.x / newScale.x;
    var newOffsetY =
      scaleCenter.y / oldScale.y + oldOffset.y - scaleCenter.y / newScale.y;
  ...
  this.setDrawings = function(
    ...
    for (var i = 0, leni = statePosGroups.length; i < leni; ++i) {
      ...
      var posGroup = drawLayer.getChildren(
        dwv.draw.isNodeWithId(statePosGroup.id())
      )[0];
  ...
  this.deleteDraws = function(cmdCallback, exeCallback) {
    ...
    while (groups.length) {
      ...
      delcmd = new dwv.tool.DeleteGroupCommand(
        groups[0],
        dwv.tool.GetShapeDisplayName(shape),
        drawLayer
      );
